...
本文档说明当前项目实际实现的推荐算法。写法参考常见论文式 Markdown：公式使用 `$$ ... $$` 数学块，步骤用文字解释，避免把公式写成代码块。
...
当前实际用到的方法包括：基于规则的安全硬过滤、口味向量相似度、HCI 健康目标知识匹配、用户偏好食材内容匹配、隐式反馈事件权重、可选 BPR 隐式反馈矩阵分解、文本向量语义匹配、内容质量评分和多样性重排。
...
菜谱健康目标信号有两种来源。直接信号来自健康目标推荐菜谱关系，间接信号来自健康目标推荐食材关系。
...
用户文本包含年龄、性别、活动水平、饮食目标、口味、健康目标、运动和偏好食材。菜谱文本包含菜谱名称、描述、菜系、做法、口味标签、食材和营养可信度。食材文本包含食材名称、类别、营养状态和来源状态。
...
其中 $C(r,s)=1$ 表示两个菜谱菜系相同，否则为 0；$M_r$ 是菜谱 $r$ 的做法集合；$G_r$ 是菜谱 $r$ 的主食材集合；$J(\cdot,\cdot)$ 是 Jaccard 相似度。
...
正反馈事件：click、save、cook。负反馈事件：skip、dislike。
